...
class ElasticPooling_Dataset(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None, analysis_mode=False, quality_ratio=[1,2,3,4]):
        """
        Dataset that manages audio recordings
        :param audio_conf: Dictionary containing the audio loading and preprocessing settings
        :param dataset_json_file
        ...
        self.max_pool_3 = nn.MaxPool1d(3, stride=3)
        self.max_pool_4 = nn.MaxPool1d(4, stride=4)
    ...
    def _wav2fbank(self, filename, filename2=None, quality=1,window_change=False):
        ...
        fbank = torchaudio.compliance.kaldi.fbank(waveform, htk_compat=True, sample_frequency=sr, use_energy=False,
                                                  window_type='hanning', num_mel_bins=self.melbins, dither=0.0)
        if quality == 2:
            fbank = self.max_pool_2(fbank.transpose(0, 1)).transpose(0, 1)
        elif quality == 3:
            fbank = self.max_pool_3(fbank.transpose(0, 1)).transpose(0, 1)
        elif quality == 4:
            fbank = self.max_pool_4(fbank.transpose(0, 1)).transpose(0, 1)

        target_length = int(self.audio_conf.get('target_length')/quality)
        target_length = target_length - target_length % 16
        # if target_length % 16 != 0:
        #     target_length = target_length + (16 - target_length % 16)
    ...
    def __getitem__(self, index):
        ...
        # FIXME: mix-up is not used for now, cannot guarantee no bug in this if branch
        if random.random() < self.mixup:
            datum = self.data[index]
            # find another sample to mix, also do balance sampling
            # sample the other sample from the multinomial distribution, will make the performance worse
