...
def pack_audio_elastic(cfg, audio_dataset, video_record, temporal_sample_index):
    samples = audio_dataset[video_record.video_id][()]
    if cfg.RANDOM and (int(round(cfg.AUDIO_DATA.SAMPLING_RATE * 2)) < video_record.num_audio_samples):
        # randomly sample a clip from 2s to the entire audio
        # precondition: the audio is at least 2s long
        clip_size = random.randint(int(round(cfg.AUDIO_DATA.SAMPLING_RATE * 2)),video_record.num_audio_samples)
    else:
        # get the entire audio
        clip_size = video_record.num_audio_samples

    
    start_idx, end_idx = get_start_end_idx(
        video_record.num_audio_samples,
        clip_size, # get the entire audio, make the clip size the same as the audio size
        temporal_sample_index,
        cfg.TEST.NUM_ENSEMBLE_VIEWS,
        start_sample=video_record.start_audio_sample
    )
    spectrogram = _extract_sound_feature_elastic(
            cfg, 
            samples, 
            video_record, 
            int(start_idx), 
...
def _extract_sound_feature_elastic(cfg, samples, video_record, start_idx, end_idx, clip_duration):
    if video_record.num_audio_samples < int(round(cfg.AUDIO_DATA.SAMPLING_RATE * clip_duration)):
        samples = samples[video_record.start_audio_sample:video_record.end_audio_sample]
    else:
        samples = samples[start_idx:end_idx]
