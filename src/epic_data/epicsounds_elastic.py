...
from .spec_augment import combined_transforms,combined_transforms_elastic
from .audio_loader_epicsounds import pack_audio_elastic
...
class Epicsounds_elastic(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        logger.info("Constructing EPIC Sounds Elastic {}...".format(mode))
    ...
    def __getitem__(self, index):
        ...
        spectrogram = pack_audio_elastic(
                self.cfg, 
                self.audio_dataset,
        ...
        if self.mode in ["train", "train+val"]:
            ...
            # SpecAugment
            spectrogram = combined_transforms_elastic(spectrogram,self.cfg)
            # C F T -> C T F
            spectrogram = spectrogram.permute(0, 2, 1)
